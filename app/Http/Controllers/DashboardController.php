...
use App\Models\{Sections, Slider, HomePageContent, Contact, Education, SignUpConference, Conferences, Activities, Tracker, Emails};
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\Mail;
use App\Mail\EducationSignUpMail;
use App\Mail\ConferenceSignUpMail;
      ...
      try {
         ...
         $data = $request->validate([
          ...
          foreach ($emails as $email) {
            App::setLocale($email->locale);
            Mail::to($email->email)->send(new KontaktMail($data));
          }
   ...
   public function storeEducationSignUp(Request $request)
   {
       try {
           ...
           $education->save();
            ...
            foreach ($emails as $email) {
               App::setLocale($email->locale);
               Mail::to($email->email)->send(new EducationSignUpMail($data));
            }
   ...
   public function storeConferenceSignUp(Request $request)
   {
      try {
         $data = $request->validate([
               ...
               'email' => 'required|email|max:255',
               'phone' => 'required|string|max:20',
               'institution_or_company_name' => 'nullable|string|max:255',
         ...
         foreach ($emails as $email) {
            App::setLocale($email->locale);
            Mail::to($email->email)->send(new ConferenceSignUpMail($data));
         }
